...
export function getTodayRecommendation(input: {
  ...
  if (!currentRoutine) {
    ...
    return weeklyCompletion.goal > weeklyCompletion.completed
      ? `이번 주 목표까지 ${weeklyCompletion.goal - weeklyCompletion.completed}회 남았어요. 오늘은 20분 러닝이나 가벼운 전신 루틴이 좋아요.`
      : '오늘 일정은 비어 있어요. 회복 스트레칭이나 산책으로 컨디션을 정리해보세요.';
  }

  const recentSessions = getRecentSessions(input.sessions, 6);
  const firstStrengthItem = currentRoutine.items.find((item) => item.kind === 'strength');

  if (firstStrengthItem) {
    const latestRecord = recentSessions
      .flatMap((session) => session.items)
      .find(
        (record) => record.kind === 'strength' && record.exerciseId === firstStrengthItem.exerciseId
      );

    const exerciseName =
      input.exercises.find((exercise) => exercise.id === firstStrengthItem.exerciseId)?.name ??
      '주요 운동';
  ...
  const runningItem = currentRoutine.items.find((item) => item.kind === 'running');

  if (runningItem) {
    const weeklyRuns = getSessionsThisWeek(input.sessions).flatMap((session) => session.items).filter((item) => item.kind === 'running').length;
    return weeklyRuns === 0
      ? '이번 주 첫 러닝을 기록해보세요. 짧아도 흐름을 살리는 데 충분합니다.'
      : '오늘 러닝은 페이스보다 호흡 리듬에 집중하면 꾸준함이 쌓입니다.';
  }
