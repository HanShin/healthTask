...
- 등과 목은 골반에서 어깨로 약 `18°` 완만하게 올라가는 중립 정렬을 유지하고 시선은 자연스럽게 아래를 향하며 골반과 어깨를 회전하지 않는다.
...
- 벤치 패드: 길이 1.22 m, 폭 0.30 m, 중심 `y=0.450`, 상단 높이 0.44 m
- 덤벨 손잡이: 지름 36 mm, 손잡이 구간 0.20 m, 전후 방향 중립 그립
- 상체: 수평보다 `18°` 높게 고정 (`rig X=72°`, root `z=0.531`)
- 덤벨 중심 이완: `x=-0.220, y=0.280, z=0.424`
- 덤벨 중심 수축: `x=-0.220, y=0.430, z=0.598`
- 왼손 지지 target: `x=0.080, y=0.140, z=0.484` (손바닥 밑면 `z=0.440` 접촉)
- 목 보정: 리그 로컬 X축 `-18°`, 머리는 로컬 X축 `+18°`로 상쇄해 척추와 평행한 중립 방향 유지
